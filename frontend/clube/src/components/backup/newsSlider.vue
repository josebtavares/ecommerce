        ...
        <swiper :pagination="pagination" :modules="modules" :autoplay="{ delay: 2000, disableOnInteraction: false }"
            ...
            <swiper-slide v-for="(item, index) in data" :key="index" 
                ...
                <div class="w-full h-full text-white relative">
                    <img :src="backendUrl+item.foto" alt="Image" class="object-cover" :style="divStyle" @click.prevent="item_clicked(item)">
                    <h3 class="font-koulen text-[20px] absolute top-[15px] right-[15px] text-shadow" @click.prevent="see_more(item)">Ler ></h3>
                    <h1 class="font-koulen text-[30px] absolute bottom-[80px] left-[25px] text-shadow">{{ item.titulo }}</h1>
                    <h1 class="font-koulen text-[20px] absolute bottom-[40px] left-[25px] text-shadow">{{ item.conteudo }}</h1>
                    <h1 class="font-koulen text-[10px] absolute bottom-[20px] left-[25px] text-shadow">{{ item.data.slice(0, 10) }}</h1>
