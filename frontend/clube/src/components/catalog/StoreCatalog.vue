  ...
  <div class="w-full">
      ...
      <button @click="selectCategoria(null)"
        :class="['px-4 py-2 rounded-full text-sm font-semibold transition-all whitespace-nowrap flex-shrink-0',
                 activeCategoria === null
                   ...
                   : isDark ? 'bg-zinc-800 text-zinc-400 hover:text-zinc-200' : 'bg-stone-200 text-zinc-600 hover:text-zinc-900']">
        Todas
      </button>
      <button v-for="cat in categorias" :key="cat.value" @click="selectCategoria(cat.value)"
        :class="['px-4 py-2 rounded-full text-sm font-semibold transition-all whitespace-nowrap flex-shrink-0 flex items-center gap-1.5',
    ...
    <div class="rounded-2xl border p-4 mb-6"
      ...
      <div class="flex flex-wrap gap-3">
        ...
        <div class="relative flex-1 min-w-48">
          ...
          <input v-model="filters.q" @input="debouncedFetch" placeholder="Pesquisar loja..."
            class="w-full pl-9 pr-3 py-2 rounded-xl text-sm border transition focus:outline-none focus:border-red-500"
            :class="isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-100 placeholder-zinc-500' : 'bg-stone-50 border-stone-300 text-zinc-900 placeholder-zinc-400'" />
        </div>
        ...
        <button @click="toggleFilter('entrega_ativa')"
          :class="['px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2',
                   filters.entrega_ativa
                     ...
                     : isDark ? 'bg-zinc-800 border border-zinc-700 text-zinc-400 hover:text-zinc-200' : 'bg-stone-100 border border-stone-300 text-zinc-600 hover:text-zinc-900']">
        ...
        <select v-model="filters.ordem" @change="resetAndFetch"
          class="px-3 py-2 rounded-xl text-sm border transition focus:outline-none focus:border-red-500 cursor-pointer"
          :class="isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-100' : 'bg-stone-50 border-stone-300 text-zinc-900'">
        ...
        <button v-if="hasActiveFilters" @click="clearFilters"
          class="px-4 py-2 rounded-xl text-sm font-semibold text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 transition-all flex items-center gap-1.5">
    ...
    <div v-if="loading && lojas.length === 0"
      ...
      <div v-for="n in 8" :key="n" class="rounded-2xl animate-pulse"
           :class="isDark ? 'bg-zinc-900' : 'bg-stone-200'"
      ...
      <div v-for="loja in lojas" :key="loja.id"
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4">
          ...
          <div class="flex gap-1.5 mt-2">
            <span v-if="loja.entrega_ativa" class="px-1.5 py-0.5 bg-green-600/20 text-green-400 text-[10px] rounded">Entrega</span>
            <span v-if="loja.levantamento_ativo" class="px-1.5 py-0.5 bg-blue-600/20 text-blue-400 text-[10px] rounded">Takeaway</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="!loading" class="text-center py-16" :class="isDark ? 'text-zinc-500' : 'text-zinc-400'">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto mb-3"
           :class="isDark ? 'text-zinc-700' : 'text-stone-300'"
           fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default {
  ...
  data () {
    return {
      ...
      activeCategoria: null,
      lojas: [], total: 0, offset: 0,
      loading: false, loadingMore: false, reachedEnd: false,
      filters: { q: '', entrega_ativa: false, levantamento_ativo: false, ordem: '' },
      debounceTimer: null, observer: null,
    }
  },
  computed: {
    hasActiveFilters () {
      return this.filters.q || this.filters.entrega_ativa || this.filters.levantamento_ativo || this.filters.ordem || this.activeCategoria
    },
  },
  async mounted () { await this.resetAndFetch(); this.setupObserver() },
  beforeUnmount () { this.observer?.disconnect(); clearTimeout(this.debounceTimer) },
  methods: {
    categoriaLabel (val) { return this.categorias.find(c => c.value === val)?.label || val },
    selectCategoria (val) { this.activeCategoria = val; this.resetAndFetch() },
    toggleFilter (key) { this.filters[key] = !this.filters[key]; this.resetAndFetch() },
    clearFilters () {
      this.filters = { q: '', entrega_ativa: false, levantamento_ativo: false, ordem: '' }
      this.activeCategoria = null; this.resetAndFetch()
    },
    ...
    buildParams (offset = 0) {
      ...
      if (this.filters.entrega_ativa)      p.entrega = 'true'
      if (this.filters.levantamento_ativo) p.levantamento = 'true'
    ...
    async resetAndFetch () {
      this.reachedEnd = false; this.offset = 0; this.lojas = []; this.loading = true
      try {
        const { data } = await api.get(this.endpoint, { params: this.buildParams(0) })
        this.lojas = data.results || data; this.total = data.count ?? this.lojas.length
        this.offset = data.next_offset ?? null; this.reachedEnd = !this.offset
    ...
    async fetchMore () {
      ...
      try {
        ...
        this.lojas.push(...(data.results || data)); this.total = data.count ?? this.lojas.length
        this.offset = data.next_offset ?? null; this.reachedEnd = !this.offset
      } catch (e) { console.error(e) }
