  ...
  <div class="w-full">
      ...
      <div v-if="categoriasVisiveis.length > 0"
        ...
        <button v-for="cat in categoriasVisiveis" :key="cat.id" @click="selectCategoria(cat)"
          :class="['px-3 py-1.5 rounded-full text-xs font-semibold transition-all whitespace-nowrap flex-shrink-0 capitalize',
                   activeCategoria?.id === cat.id ? 'bg-red-600/80 text-white' : 'bg-zinc-800/80 text-zinc-400 hover:text-zinc-200']">
          {{ cat.icone }} {{ cat.nome }}
        </button>
      </div>
    </transition>
    ...
    <div v-if="activeTipo || activeCategoria" class="flex items-center gap-2 mb-4 text-xs text-zinc-500">
      ...
      <span v-if="activeCategoria" class="px-2 py-0.5 bg-zinc-700 text-zinc-300 rounded-full capitalize">
        {{ activeCategoria.icone }} {{ activeCategoria.nome }}
      </span>
      <button @click="resetAll" class="text-zinc-600 hover:text-zinc-400 transition ml-1">× limpar</button>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      tipos: [],
      activeTipo: null,
      todasCategorias: [],   // CategoriaLoja objects {id, nome, icone}
      categoriasDoTipo: [],  // filtradas para o tipo activo
      activeCategoria: null, // CategoriaLoja object ou null
      produtos: [],
      total: 0,
      offset: 0,
      loading: false,
  ...
  computed: {
    // mostra sub-abas de categorias se: tipo seleccionado (categorias desse tipo) ou todas as categorias
    categoriasVisiveis () {
      if (this.activeTipo) return this.categoriasDoTipo
  ...
  methods: {
    ...
    async fetchCategoriasDoTipo (tipo) {
      ...
      try {
        // filtra categorias que têm produtos do tipo seleccionado
        ...
        produtos.forEach(p => {
          p.categorias?.forEach(cat => {
            if (!catIds.has(cat.id)) { catIds.add(cat.id); cats.push(cat) }
          })
        })
    ...
    buildParams (offset = 0) {
      ...
      if (this.activeCategoria)           p.categoria_id = this.activeCategoria.id  // ← ID, não nome
