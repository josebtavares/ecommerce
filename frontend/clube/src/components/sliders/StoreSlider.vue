  ...
  <div v-if="lojas.length > 0 || loading" class="mb-10">
    ...
    <div v-if="loading && lojas.length === 0" class="flex gap-4 overflow-hidden">
      <div v-for="n in 4" :key="n"
           class="flex-shrink-0 bg-zinc-800 rounded-2xl animate-pulse" style="width:260px;height:200px"></div>
    </div>
    ...
    >
      <swiper-slide v-for="loja in lojas" :key="loja.id" class="!w-64">
        <div
          ...
          class="group relative rounded-2xl overflow-hidden cursor-pointer bg-zinc-800 h-60
                 hover:ring-2 hover:ring-red-500/50 transition-all"
        >
          <img
          ...
          <div class="absolute top-3 left-3">
            ...
            <div v-else class="w-10 h-10 rounded-xl bg-zinc-700 flex items-center justify-center border-2 border-zinc-600">
          ...
          <div class="absolute bottom-0 left-0 right-0 p-3">
            ...
            <p v-if="loja.localizacao" class="text-xs text-zinc-400 truncate mt-0.5">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 inline mr-0.5 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <swiper-slide v-if="loadingMore" class="!w-64">
        <div class="w-64 h-48 bg-zinc-800 rounded-2xl animate-pulse flex-shrink-0"></div>
      </swiper-slide>
    </swiper>
  </div>
</template>
