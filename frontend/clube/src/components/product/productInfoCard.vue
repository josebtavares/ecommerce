<template>
  <transition enter-active-class="transition duration-200" enter-from-class="opacity-0"
    ...
    <div v-if="produto"
      ...
      <!-- Mobile: card vertical (original) | md+: dois painéis lado a lado -->
      <div class="relative w-full max-w-2xl md:max-w-4xl max-h-[90vh] rounded-2xl overflow-hidden shadow-2xl border flex flex-col md:flex-row"
        ...
        <!-- Mobile: altura fixa 256px. md+: ocupa metade, altura total -->
        <div class="relative md:w-1/2 md:flex-shrink-0 h-64 md:h-auto overflow-hidden group cursor-pointer"
             @click="produto.ficheiro_url && (imagemFullscreen = true)">

          <img v-if="produto.ficheiro_url"
               :src="produto.ficheiro_url" :alt="produto.nome"
               class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
          <div v-else class="w-full h-full flex items-center justify-center"
               :class="isDark ? 'bg-zinc-800' : 'bg-gray-100'">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16"
                 :class="isDark ? 'text-zinc-600' : 'text-gray-300'"
          ...
          <!-- Botão fechar — só visível no mobile (no desktop está no painel direito) -->
        ...
        <div class="md:w-1/2 flex flex-col overflow-y-auto max-h-[calc(90vh-256px)] md:max-h-[90vh]">
          ...
          <!-- Header do painel direito com botão fechar (desktop) -->
          <div class="flex items-center justify-between px-6 pt-5 pb-2 flex-shrink-0">
            ...
            <div class="flex items-center gap-2">
              ...
              <span v-else class="text-xs font-medium"
                    :class="isDark ? 'text-zinc-500' : 'text-zinc-400'">{{ loja?.nome }}</span>
          ...
          <div class="px-6 pb-6 flex-1 overflow-y-auto">
            ...
            <h2 class="text-2xl font-bold mb-1"
                :class="isDark ? 'text-zinc-100' : 'text-zinc-900'">{{ produto.nome }}</h2>
            ...
            <!-- Choices (atributos seleccionáveis) -->
            ...
            <div class="flex items-center gap-4 mb-5">
              ...
              <div class="flex items-center gap-3">
                <button @click="qty > 1 && qty--"
                  ...
                  class="w-8 h-8 rounded-lg flex items-center justify-center transition disabled:opacity-40 disabled:cursor-not-allowed"
                  :class="isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-gray-100 hover:bg-gray-200'">
                ...
                <span class="text-lg font-bold w-8 text-center"
                      :class="isDark ? 'text-zinc-100' : 'text-zinc-900'">{{ qty }}</span>
...
export default {
  ...
  data () {
    return { qty: 1, seleccoes: {}, imagemFullscreen: false }
  },
  ...
  computed: {
    ...
    obrigatoriosPorPreencher () {
      return this.atributosChoices.filter(a => a.obrigatorio && !this.seleccoes[a.nome]).map(a => a.nome)
    },
    podeAdicionar () { return this.obrigatoriosPorPreencher.length === 0 },
  },

  watch: {
    produto () { this.qty = 1; this.seleccoes = {}; this.imagemFullscreen = false },
  },
  ...
  methods: {
    ...
    toggleSeleccao (nome, opcao) {
      if (this.seleccoes[nome] === opcao) {
        const novas = { ...this.seleccoes }; delete novas[nome]; this.seleccoes = novas
      } else {
