<template>
  <div class="space-y-5">
    ...
    <p class="text-xs text-zinc-600">
      ...
      <span v-if="pesquisa"> para "{{ pesquisa }}"</span>
    </p>
    ...
    <div v-else-if="produtos.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      <div v-for="p in produtos" :key="p.id"
        ...
        <div class="p-3">
          ...
          <p class="text-xs text-zinc-500 mt-0.5">{{ p.tipo?.nome || '—' }}</p>
    ...
    <div v-else class="text-center py-16 text-zinc-500 text-sm bg-zinc-900 rounded-2xl border border-zinc-800">
      Nenhum produto encontrado.
      <button @click="abrirCriar" class="text-red-400 hover:text-red-300 ml-2">Criar o primeiro →</button>
    </div>
...
export default {
  ...
  async created () { await this.fetchProdutos() },
  ...
  methods: {
    ...
    onSaved () { this.showModal = false; this.fetchProdutos() },
  }
}
</script>