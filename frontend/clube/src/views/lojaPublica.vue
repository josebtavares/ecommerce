  ...
  <div class="min-h-screen bg-zinc-950 text-zinc-100">
    ...
    <template v-else-if="loja">
      ...
      <div class="flex">
        ...
        <!-- ── SIDEBAR (só aparece se tiver muitos tipos/categorias) ── -->
        <transition enter-active-class="transition duration-300" enter-from-class="-translate-x-full"
          ...
          <aside v-if="sidebarAberta"
            ...
            <div v-if="categoriasExistentes.length > 0" class="px-3 mb-5 border-t border-zinc-800 pt-4">
              ...
              <button
                v-for="cat in categoriasExistentes" :key="cat"
                @click="scrollToId('cat-' + cat); sidebarAberta = false"
                class="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-zinc-400
                       hover:bg-zinc-800 hover:text-zinc-100 transition text-left">
                <span>📂</span>
        ...
        <!-- Botão toggle sidebar (só aparece se tiver sidebar) -->
        ...
        <main class="flex-1 min-w-0">
          <div class="max-w-6xl mx-auto px-6 py-8">
            ...
            <template v-if="categoriasExistentes.length > 0">
              ...
              <div v-for="cat in categoriasExistentes" :key="'cat-' + cat" :id="'cat-' + cat">
                <ProductSlider
                  :title="cat" icon="📂"
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                ...
                <div v-if="loja.politica_devolucao" class="space-y-2">
                  <button @click="modalPolitica = 'devolucao'"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-1.5 4 1.5 4-1.5 4 1.5z" />
                    </svg>
                    Política de devoluções
                  </button>
                  <button v-if="loja.termos_servico" @click="modalPolitica = 'termos'"
                    class="text-sm font-semibold text-zinc-400 hover:text-zinc-200 transition flex items-center gap-2 block">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default {
  ...
  computed: {
    // Sidebar aparece só se tiver pelo menos 3 tipos ou 3 categorias
    temSidebar () {
      return (this.tiposExistentes.length + this.categoriasExistentes.length) >= 3
    },

    temFooter () {
      return this.loja?.politica_devolucao ||
             this.loja?.termos_servico ||
             this.loja?.politica_privacidade ||
             this.metodosPagamento.length > 0
    },
  },
