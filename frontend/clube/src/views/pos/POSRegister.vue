  ...
  methods: {
    ...
    validateForm() {
      ...
      if (this.form.password.length < 6) {
        return 'A password deve ter no mínimo 6 caracteres.'
      }
    ...
    persistSession(data) {
      ...
      // Contexto específico do módulo POS
    ...
    resolvePOSList(data) {
      if (Array.isArray(data.pos_existentes)) {
        return data.pos_existentes
      }
    ...
    getErrorMessage(err) {
      ...
      if (err.response?.status === 400) {
        return 'Email já registado ou dados inválidos.'
      }

      if (err.response?.status === 401) {
        return 'Não foi possível autenticar esta conta.'
      }
